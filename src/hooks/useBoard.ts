...
export const getPreviousIncompleteActions = async (userId: string, currentBoardId: string) => {
    try {
        // 1. Get user's boards strictly created by them for now, or use complex index for team
        // Assuming simple case: previous boards with same owner
        const boardsQuery = query(
            collection(db, "boards"),
            where("createdBy", "==", userId),
            orderBy("createdAt", "desc"),
            limit(10) // Check last 10 boards
        );
        
        const boardsSnap = await getDocs(boardsQuery);
        let previousActions: RetroCard[] = [];

        // Iterate boards to find pending actions
        // Start from index 0? No, skip current board if it's in the list
        for (const doc of boardsSnap.docs) {
            if (doc.id === currentBoardId) continue;

            const cardsQuery = query(
                collection(db, `boards/${doc.id}/cards`),
                where("isActionItem", "==", true),
                where("isDone", "==", false) // Not marked as done
            );
            ...
            const cards = cardsSnap.docs.map(c => ({ 
                id: c.id, 
                ...c.data(), 
                boardId: doc.id, // Track origin
                boardName: doc.data().name 
            } as any));
            
            previousActions = [...previousActions, ...cards];
        }

        return previousActions;
    } catch (e) {
        console.error("Error fetching previous actions", e);
        return [];
    }
};
